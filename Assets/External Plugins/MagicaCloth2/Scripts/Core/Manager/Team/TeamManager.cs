        ...
        struct AlwaysTeamUpdateJob : IJob
        {
            ...
            public void Execute()
            {
                ...
                for (int teamId = 0; teamId < teamCount; teamId++)
                {
                    ...
                    // 周波数切り替えにより、時間が不正な値になった場合の対処
                    if ((tdata.nowUpdateTime + simulationDeltaTime) < tdata.time)
                    {
                        // 前回のステップ時間にデルタタイムを加えても、前回の進行時間を超えなくなった状況
                        // これは実行時に周波数が上がり、デルタタイムが短くなった場合に発生する

                        // 次のステップの時間に合わせるように、過去のフレーム時間を押し上げることで調整する
                        float otime = tdata.nowUpdateTime + simulationDeltaTime;
                        tdata.time = otime;
                        if (tdata.frameUpdateTime > otime)
                            tdata.frameUpdateTime = otime;
                    }
